...
function requireMain$1() {
  ...
  (function(exports) {
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.UpdaterSignal = exports.UPDATE_DOWNLOADED = exports.DOWNLOAD_PROGRESS = exports.NsisUpdater = exports.MacUpdater = exports.RpmUpdater = exports.PacmanUpdater = exports.DebUpdater = exports.AppImageUpdater = exports.Provider = exports.CancellationToken = exports.NoOpLogger = exports.AppUpdater = exports.BaseUpdater = void 0;
    const builder_util_runtime_1 = requireOut();
    Object.defineProperty(exports, "CancellationToken", { enumerable: true, get: function() {
...
function logFiles() {
  ...
  fileList.forEach((file2, index) => {
    log.info(`
[${index + 1}] 文件信息:`);
    log.info(`  - 文件ID: ${file2.fileId}`);
    log.info(`  - 文件名: ${file2.name}`);
    log.info(`  - MIME类型: ${file2.mimeType}`);
    log.info(`  - 服务器路径: ${file2.server_path}`);
    if (file2.content) {
      if (typeof file2.content === "string") {
        log.info(`  - 文件内容: ${file2.content.substring(0, 50)}...`);
      } else if (Buffer.isBuffer(file2.content)) {
        log.info(`  - 文件大小: ${file2.content.length} 字节`);
      } else {
        log.info("  - 文件内容: 格式未知");
      }
    } else {
...
async function handleFile(fileInfo) {
  ...
  try {
    ...
    let filesDir = require$$1$4.join(__dirname, "files");
    if (require$$1$5.app.isPackaged) {
      filesDir = require$$1$4.join(require$$1$5.app.getPath("userData"), "files");
    }
    if (!fs$1.existsSync(filesDir)) {
      fs$1.mkdirSync(filesDir, { recursive: true });
    }
    log.info("文件存储目录:", filesDir);
    const targetPath = require$$1$4.join(filesDir, processedFile.name);
    await fs$1.promises.writeFile(targetPath, processedFile.content || "");
    ...
    console.error(`处理文件 ${fileInfo.name} 时出错:`, error2);
    return null;
  }
}
...
require$$1$5.app.whenReady().then(() => {
  ...
  win.loadFile(require$$1$4.join(__dirname, "../renderer/index.html"));
  require$$1$5.ipcMain.on("file:selected", async (_event, fileInfo) => {
    await handleFile(fileInfo);
  });
  require$$1$5.ipcMain.on("file:removed", async (_event, fileInfo) => {
    try {
      const fileToDelete = fileList.find((file2) => file2.fileId === fileInfo.fileId);
      ...
      fileList = fileList.filter((file2) => file2.fileId !== fileInfo.fileId);
      logFiles();
    } catch (error2) {
      log.error("删除文件失败:", error2);
    }
  });
  require$$1$5.ipcMain.handle("dialog:openFile", async () => {
    const result = await require$$1$5.dialog.showOpenDialog(win, {
      properties: ["openFile", "multiSelections"]
    });
    return result.filePaths;
  });
  require$$1$5.ipcMain.handle("file:read", async (_event, filePath) => {
    try {
      if (!filePath) {
        log.error("文件路径为空");
...
错误详情: ${errorMessage}
${error2 instanceof Error ? error2.stack : ""}`);
      throw error2 instanceof Error ? error2 : new Error(`读取文件失败: ${errorMessage}`);
    }
  });
  require$$1$5.ipcMain.handle("system:info", () => {
    return {
      platform: require$$1$1.platform(),
      cpu: require$$1$1.loadavg()[0],
      mem: require$$1$1.totalmem() - require$$1$1.freemem()
    };
  });
});
    ...
    require$$1$5.app.whenReady().then(() => {
      const win = new require$$1$5.BrowserWindow({
        ...
        webPreferences: {
          nodeIntegration: false,
          contextIsolation: true,
          preload: require$$1$4.join(__dirname, "../preload/preload.js")
        }
      });
      win.loadFile(require$$1$4.join(__dirname, "../renderer/index.html"));
    });
